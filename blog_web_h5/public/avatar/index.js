/*
 * @Author: sun
 * @Date: 2022-12-31 09:48:43
 * @LastEditTime: 2022-12-31 09:48:53
 * @Description: Do not edit
 */
export function multiavatar(string, sansEnv, ver) {
  string += "";
  var themes = {
      "00": {
        A: {
    ...
    var a = [],
      f = (f.SHA256 = g.extend({
        ...
        _doFinalize: function () {
          ...
          return (
            (d[e >>> 5] |= 128 << (24 - (e % 32))),
            (d[14 + (((e + 64) >>> 9) << 4)] = h.floor(b / 4294967296)),
            (d[15 + (((e + 64) >>> 9) << 4)] = b),
  ...
  var p = [];
  for (var part in ((p.env = hash[0] + "" + hash[1]),
  (p.env = Math.round(0.47 * p.env) + ""),
  (p.clo = hash[2] + "" + hash[3]),
  (p.clo = Math.round(0.47 * p.clo) + ""),
  (p.head = hash[4] + "" + hash[5]),
  (p.head = Math.round(0.47 * p.head) + ""),
  (p.mouth = hash[6] + "" + hash[7]),
  (p.mouth = Math.round(0.47 * p.mouth) + ""),
  (p.eyes = hash[8] + "" + hash[9]),
  (p.eyes = Math.round(0.47 * p.eyes) + ""),
  (p.top = hash[10] + "" + hash[11]),
  (p.top = Math.round(0.47 * p.top) + ""),
  p)) {
    var nr = p[part];
    nr > 31
      ? (1 == (nr = nr - 32 + "").length && (nr = "0" + nr), (p[part] = nr + "C"))
      : nr > 15
      ? (1 == (nr = nr - 16 + "").length && (nr = "0" + nr), (p[part] = nr + "B"))
      : (p[part] = 1 == (nr + "").length ? "0" + nr + "A" : nr + "A");
  }
  var final = [];
  for (var part in p) {
  ...
  return (
    sansEnv && (final.env = ""),
    svgStart + final.env + final.head + final.clo + final.top + final.eyes + final.mouth + svgEnd
  );
}
